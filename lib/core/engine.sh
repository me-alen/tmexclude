...
tmx_collect_candidates_for_project() {
  ...
  local scanner
  for scanner in $(tmx_detect_scanners_for_root "${project_root}"); do
    ...
    done < <(tmx_scanner_candidates "${scanner}")
  done
}
...
tmx_collect_candidates_for_configured_roots() {
  ...
  for root in "${TMX_ROOT_PATHS[@]}"; do
    while IFS= read -r maybe_project; do
      local scanner_hits
    ...
    done < <(tmx_find_project_roots "${root}")
  done | sort -u
}
